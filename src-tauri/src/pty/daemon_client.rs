...
pub type OnAgentStatus = Arc<dyn Fn(&str, &crate::agents::status::AgentStatusEntry) + Send + Sync>;
...
pub struct DaemonClient {
    ...
    agent_status_cb: Arc<Mutex<Option<OnAgentStatus>>>,
...
impl DaemonClient {
    ...
    pub fn connect(socket_path: &str) -> Result<Self, String> {
        ...
        let agent_status_cb: Arc<Mutex<Option<OnAgentStatus>>> = Arc::new(Mutex::new(None));
        ...
        let agent_status_cb_clone = Arc::clone(&agent_status_cb);
                    ...
                    Ok(_) => {
                        ...
                        if let Ok(event) = serde_json::from_str::<DaemonEvent>(trimmed) {
                            match event {
                                ...
                                DaemonEvent::AgentStatusChanged { pane_key, entry } => {
                                    if let Some(cb) = agent_status_cb_clone.lock().as_ref() {
                                        cb(&pane_key, &entry);
                                    }
                                }
                            }
    ...
    /// Register a global hook fired whenever an agent posts a classified status entry.
    pub fn set_agent_status_callback(&self, cb: OnAgentStatus) {
        *self.agent_status_cb.lock() = Some(cb);
    }
    ...
    pub fn ack(&self, session_id: &str, bytes: usize) -> Result<(), String> {
        // WHY emit under the legacy `chars` key (see the Ack field attrs):
        // pre-v7 daemons (0.2.2 and earlier) parse only `chars` with no alias,
        // so a bytes-keyed ack from an updated GUI would wedge them. Never add
        // a second key — dual-key payloads fail serde parsing on every version.
        Self::queue_request(
            &self.write_tx,
            &self.connected,
            DaemonRequest::Ack {
