...
// frontend learn whether this binary is the dev ("Developer OPPA") or stable
// build, so later tasks (updater gating, banner) can branch on it without
// another IPC surface. The payload is just `Channel::current().as_str()`.

use crate::channel::Channel;

/// Return the compile-time build channel: `"dev"` or `"stable"`.
#[tauri::command]
...
mod tests {
    ...
    fn app_channel_payload_matches_build_time_channel() {
        ...
        assert!(payload == "dev" || payload == "stable");
    }

    #[test]
    fn app_channel_payload_serializes_to_plain_string() {
        // Tauri serializes the returned &str as a JSON string on the wire; a
        // payload like `"dev"` / `"stable"` is exactly what the frontend's
        // `invoke<string>("app_channel")` expects.
