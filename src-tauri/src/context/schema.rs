...
fn seed_builtin_personas(conn: &Connection) -> rusqlite::Result<()> {
    ...
    let mut stmt = conn.prepare_cached(
        ...
        ) VALUES (?1, 'global', 'persona', ?2, ?3, ?4, ?5, ?6, '[]', 0, ?7, ?7)
        "#,
    )?;
...
mod tests {
    ...
    fn test_built_in_personas_seeded() {
        ...
        let mut stmt = conn
            .prepare("SELECT id, title, icon, category, scope FROM context_pages WHERE category='persona' ORDER BY id")
            .unwrap();
        let personas: Vec<(String, String, String, String, String)> = stmt
            .query_map([], |row| {
                Ok((
                    row.get(0)?,
                    row.get(1)?,
